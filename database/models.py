...
class BuyerModel(Base):
    """
    Buyer entity shared with Ishan's marketplace module.
    """
    ...
    latitude = Column(Float, nullable=True)
    longitude = Column(Float, nullable=True)
    preferred_commodities = Column(JSON, default=lambda: ["Wheat"])
    accepted_grades = Column(JSON, default=lambda: ["Grade A"])
    min_quantity = Column(Float, default=10.0)
    max_quantity = Column(Float, default=500.0)
    base_offer_price = Column(Float, default=2570.0)
    reliability_score = Column(Float, default=0.95)  # 0.0 - 1.0 (on-time payment)
    delivery_preference = Column(String(32), default="FARMGATE")  # FARMGATE, MANDI_DELIVERY
    dispute_free_trades = Column(Integer, default=45)
    total_trades = Column(Integer, default=46)
    verification_status = Column(String(32), default="VERIFIED")
    created_at = Column(DateTime, default=utc_now)


class BuyerMatchModel(Base):
    """
    Persisted buyer match rankings generated by Kuldeep's intelligence engine.
    """
    __tablename__ = "buyer_matches"

    id = Column(String(64), primary_key=True, index=True)
    crop_lot_id = Column(String(64), ForeignKey("crop_lots.id"), nullable=False, index=True)
    buyer_id = Column(String(64), ForeignKey("buyers.id"), nullable=False, index=True)
    match_score = Column(Float, nullable=False)
    offer_price = Column(Float, nullable=False)
    estimated_net_realisation = Column(Float, nullable=False)
